...
export function ExecutiveQuickLinksManager({
  ...
  const fetchLinks = async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/executives/${executiveId}/quick-links`);
  ...
  const handleSubmit = async (e?: React.FormEvent | React.MouseEvent | React.KeyboardEvent) => {
    ...
    try {
      ...
      const url = editingId
        ? `/api/executives/quick-links/${editingId}`
        : `/api/executives/${executiveId}/quick-links`;
  ...
  const deleteLink = async (linkId: string) => {
    try {
      const response = await fetch(`/api/executives/quick-links/${linkId}`, {
        method: 'DELETE'
      });
  ...
  const saveReorder = async () => {
    try {
      ...
      const response = await fetch('/api/executives/quick-links/reorder', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
