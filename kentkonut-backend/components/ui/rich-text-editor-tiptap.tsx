...
export default function RichTextEditor({
  ...
  const applyFontSize = useCallback((fontSize: string) => {
    console.log('🎯 applyFontSize called with:', fontSize);
    console.log('📝 Current editor state:', editor?.getHTML());
    
    if (fontSize === 'default') {
      // Only remove fontSize, keep other textStyle attributes
      const currentAttributes = editor?.getAttributes('textStyle') || {};
      console.log('📋 Current textStyle attributes:', currentAttributes);
      const { fontSize: _, ...otherAttributes } = currentAttributes;
      
      if (Object.keys(otherAttributes).length > 0) {
        console.log('✅ Setting textStyle with other attributes:', otherAttributes);
        editor?.chain().focus().setMark('textStyle', otherAttributes).run();
      } else {
        console.log('🗑️ Removing textStyle mark completely');
        editor?.chain().focus().unsetMark('textStyle').run();
      }
    } else {
    ...
    setTimeout(() => {
      console.log('📄 HTML after font size change:', editor?.getHTML());
    }, 100);
  }, [editor]);
