...
export default async function PersonnelDetail({ params }: Props) {
  ...
  const imageGalleryItems = person.galleryItems.filter(item => item.type === 'IMAGE');
  const documentGalleryItems = person.galleryItems.filter(item => ['DOCUMENT', 'PDF', 'WORD'].includes(item.type));
