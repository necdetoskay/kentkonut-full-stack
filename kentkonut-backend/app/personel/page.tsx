...
export default async function PersonnelPage() {
  ...
  const directors = personnel.filter(person => person.type === 'DIRECTOR');
  const chiefs = personnel.filter(person => person.type === 'CHIEF');
                ...
                <div className="p-4">
                  ...
                  {chief.chiefInDepts.length > 0 && (
                    <div className="text-xs text-gray-600">
                      ...
                      <span> {chief.chiefInDepts.map(dept => dept.name).join(', ')}</span>
                    </div>
                  )}
                </div>
              </Link>
