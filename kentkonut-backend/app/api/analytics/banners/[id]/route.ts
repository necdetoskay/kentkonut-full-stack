...
export async function GET(
  ...
  try {
    ...
    const performanceSummary = await prisma.bannerPerformanceSummary.findMany({
      where: {
        bannerId,
        date: {
          gte: startDate,
    ...
    const avgEngagementTime = analytics
      .filter(a => a.engagementTime && a.engagementTime > 0)
      .reduce((sum, a) => sum + (a.engagementTime || 0), 0) / 
      Math.max(1, analytics.filter(a => a.engagementTime && a.engagementTime > 0).length);
...
export async function POST(
  ...
  try {
    ...
    const analyticsRecord = await prisma.bannerAnalytics.create({
      data: {
        bannerId,
        sessionId,
        userId,
