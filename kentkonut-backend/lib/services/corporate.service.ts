...
import { ExecutiveType, CorporateContentType } from "@prisma/client";

export class CorporateService {
  // Yöneticiler
  static async getExecutives(type?: ExecutiveType) {
  ...
  static async getPresident() {
    return await db.executive.findFirst({
      where: { type: 'PRESIDENT', isActive: true }
    });
  }

  static async getGeneralManager() {
    return await db.executive.findFirst({
      where: { type: 'GENERAL_MANAGER', isActive: true }
    });
  }
