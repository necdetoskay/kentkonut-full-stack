...
export function renderContentToHTML(content: string): string {
  try {
    ...
    const htmlParts = contentData.blocks
      ...
      .map(block => {        // Farklı blok tiplerini destekle
        switch (block.type) {          case 'text':
            const textContent = block.content || '';
            return textContent ? `
              <div style="margin-bottom: 40px; line-height: 1.7; font-size: 16px; clear: both; display: block; overflow: hidden; width: 100%;">
                ${textContent}
              </div>
            ` : '';case 'image':
            ...
            if (images.length > 0) {
              ...
              const spacingValue = spacing === 'compact' ? '8px' : spacing === 'relaxed' ? '16px' : '12px';
              
              // Calculate fixed thumbnail size
              const thumbnailSize = columns <= 3 ? '160px' : columns === 4 ? '140px' : '120px';
              
              // Create thumbnail gallery with flexbox - NO margin-bottom!
              ...
              `).join('');
                ...
                <div style="margin: 40px 0; clear: both; display: block; overflow: hidden; width: 100%; position: relative;">
                  ...
                  <div style="
                    display: grid;
                    grid-template-columns: repeat(${columns}, ${thumbnailSize});
                    gap: ${spacingValue};
