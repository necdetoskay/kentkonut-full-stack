...
export function BannerAPITest() {
  ...
  const runTests = async () => {
    ...
    try {
      const directResponse = await fetch('http://localhost:3010/api/health');
    ...
    try {
      const directBannerResponse = await fetch(`http://localhost:3010/api/public/banners/position/${BANNER_POSITION_UUIDS.HERO}`);
    ...
    try {
      const corsResponse = await fetch(`http://localhost:3010/api/public/banners/position/${BANNER_POSITION_UUIDS.HERO}`, {
        method: 'OPTIONS',
        headers: {
    ...
    <div className="p-6 max-w-4xl mx-auto">
      <div className="mb-6">
        ...
        <div className="mt-4">
          ...
          <p><strong>Backend URL:</strong> {import.meta.env.VITE_API_BASE_URL || 'http://localhost:3010'}</p>
