...
// Get API base URL - fallback to localhost for development
const getApiBaseUrl = (): string => {
  // Try import.meta.env for Vite (this is the standard way in Vite)
  if (import.meta.env && import.meta.env.VITE_API_URL) {
    return import.meta.env.VITE_API_URL as string;
  }

  // Try to get from window object (for runtime configuration)
...
class ServiceCardService {
  ...
  async trackCardClick(cardId: number): Promise<boolean> {
    try {
      ...
      console.error('[ServiceCardService] Error tracking click:', error);
      return false;
    }
  }
