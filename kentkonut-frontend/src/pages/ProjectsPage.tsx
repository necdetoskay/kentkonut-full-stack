...
const ProjectsPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchOngoing = async (search = '') => {
      try {
        const res = await fetch(`http://localhost:3010/api/projects?status=ONGOING&limit=12${search ? `&search=${encodeURIComponent(search)}` : ''}`);
    ...
    const fetchCompleted = async (search = '') => {
      try {
        const res = await fetch(`http://localhost:3010/api/projects?status=COMPLETED&limit=12${search ? `&search=${encodeURIComponent(search)}` : ''}`);
        if (!res.ok) throw new Error('Projeler alınamadı');
        const data = await res.json();
    ...
    const fetchAll = async (search = '') => {
      try {
        const res = await fetch(`http://localhost:3010/api/projects?limit=24${search ? `&search=${encodeURIComponent(search)}` : ''}`);
        if (!res.ok) throw new Error('Projeler alınamadı');
        const data = await res.json();
  ...
  useEffect(() => {
    ...
    if (activeTab === 'ongoing') {
      fetch(`http://localhost:3010/api/projects?status=ONGOING&limit=12&search=${encodeURIComponent(searchTerm)}`)
        .then(res => res.json())
        .then(data => setOngoingProjects(data.projects || []));
    } else if (activeTab === 'completed') {
      fetch(`http://localhost:3010/api/projects?status=COMPLETED&limit=12&search=${encodeURIComponent(searchTerm)}`)
        .then(res => res.json())
        .then(data => setCompletedProjects(data.projects || []));
    } else if (activeTab === 'all') {
      fetch(`http://localhost:3010/api/projects?limit=24&search=${encodeURIComponent(searchTerm)}`)
        .then(res => res.json())
  ...
  return (
    <div className="min-h-screen bg-white">
      {/* Hero Section */}
      <div className="relative h-80 bg-kentblue">
        <div 
        ...
        <div className="relative h-full flex flex-col items-center justify-center px-4 z-10">
          <h1 className="text-4xl md:text-5xl font-bold text-white mb-4 text-center text-shadow">
            Projelerimiz
          </h1>
          <p className="text-lg text-white max-w-2xl text-center text-shadow">
            Kent Konut olarak hayata geçirdiğimiz tüm prestijli projelerimiz. Sizin için tasarladığımız 
